...
frame_buffer = [] # Lista para armazenar as contagens recentes
stable_total_fingers = 0 # O número de dedos estabilizado
...
while True:
    ...
    dedos = [0, 0, 0, 0, 0]
    current_total_fingers = 0 # Contagem deste frame específico
    ...
    if results.multi_hand_landmarks and results.multi_handedness:
        ...
        # 1. Contar quantos dedos estão levantados (quantos '1' existem na lista 'dedos')
    ...
    # --- 3. LÓGICA DO FILTRO DE ESTABILIZAÇÃO ---
    
    # Adiciona a contagem atual ao buffer
    frame_buffer.append(current_total_fingers)
    
    # Mantém o buffer com o tamanho máximo (BUFFER_SIZE)
    if len(frame_buffer) > BUFFER_SIZE:
        frame_buffer.pop(0) # Remove o frame mais antigo

    # Só atualiza a contagem estável se o buffer estiver cheio
    if len(frame_buffer) == BUFFER_SIZE:
        # Conta a ocorrência de cada número no buffer
        # Ex: [1, 2, 2, 2, 1] -> Counter({2: 3, 1: 2})
        count_data = Counter(frame_buffer)
        
        # Pega o número mais comum. Ex: 2
        most_common_count = count_data.most_common(1)[0][0]
        
        # Pega a frequência desse número. Ex: 3
        most_common_freq = count_data.most_common(1)[0][1]
        
        # Só atualiza se o número mais comum aparecer em mais da metade
        # do buffer (previne mudanças abruptas)
        if most_common_freq > BUFFER_SIZE // 2:
             stable_total_fingers = most_common_count
    
    # --- MOSTRAR O NÚMERO TOTAL DE DEDOS (ESTABILIZADO) ---
    
    # Escrever o número total ESTÁVEL na tela
    # (Coloca um fundo preto para legibilidade)
    cv2.putText(img, str(stable_total_fingers), (30, 75), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 0), 10)
    # (Coloca o texto branco por cima)
    cv2.putText(img, str(stable_total_fingers), (30, 75), cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 255, 255), 5)
    ...
    if frame_count % 10 == 0:
        ...
        print(f"Estável: {stable_total_fingers}, Bruto: {current_total_fingers}, Buffer: {frame_buffer}")
    frame_count += 1
